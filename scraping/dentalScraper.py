...
def getAIAData(name, birth, gender):
    ...
    scrapingResult = {"company": "AIA", "price": 0, "contents": []}
    ...
    tableBody = driver.find_element_by_xpath(
        '//*[@id="collapse-large-724022276"]/div[1]/div/table'
    ).find_element_by_tag_name("tbody")
    driver.find_element_by_xpath(
        '//*[@id="the_fine_print"]/div[2]/div[1]/div[2]/div/a[2]'
    ).click()
...
def getLinaData(name, birth, gender):
    driver = webdriver.Chrome("./chromedriver")
    scrapingResult = {"company": "라이나", "price": 0, "contents": []}
    ...
    htmlResult = driver.find_element_by_xpath(
        '//*[@id="contents"]/div[2]/div[2]/div[2]/div/table/tbody[1]/tr[1]/td[2]/strong'
    ).text
    resultValue = rePlaceData(htmlResult)
    scrapingResult["price"] = resultValue
    ...
    tableBody = driver.find_element_by_xpath(
        '//*[@id="planPonA2"]/div/div[2]/div/div/table[1]'
    ).find_element_by_tag_name("tbody")
    rows = tableBody.find_elements_by_tag_name("tr")
    contentsList = []
    for index, value in enumerate(rows):
